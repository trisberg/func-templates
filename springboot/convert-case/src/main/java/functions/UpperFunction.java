...
@Component("upper")
public class UpperFunction implements Function<Message<String>, Message<String>> {
  private static final Logger LOGGER = Logger.getLogger(
      UpperFunction.class.getName());
  ...
  public Message<String> apply(Message<String> inputMessage) {
     ...
     */
     ...
     LOGGER.log(Level.INFO, "Input CE Type:{0}", httpHeaders.getFirst(TYPE));
     ...
     return CloudEventMessageBuilder.withData(input.toUpperCase())
        .setType(httpHeaders.getFirst(TYPE)).setId(UUID.randomUUID().toString())
        .setSubject("Echo event")
        .setSource(URI.create("http://example.com/upper")).build();
  }
}
